...
fn cognition_receipt_round_trips_all_commit_evidence() {
    ...
    let recovered: CognitionCommitReceipt = serde_json::from_slice(&persisted).unwrap();
    assert_eq!(
        token,
        issuer
            .issue_cognition(&recovered, recovered.issued_at() + TimeDelta::seconds(1))
            .unwrap()
    );
    let verified = ReceiptVerifier::new(issuer.verifying_key())
        .verify_cognition(&token, receipt.issued_at() + TimeDelta::seconds(1))
        .unwrap();
...
#[test]
fn local_v3_receipt_may_omit_the_optional_governed_scope() {
    let receipt = receipt();
...
fn wire_distinguishes_revalidation_preparation_commit_and_expiry() {
    ...
    assert_eq!(
        encoded.get("issuedAt").and_then(|value| value.as_str()),
        Some("2026-08-05T12:00:03Z")
    );
    assert_eq!(
        receipt.expires_at(),
    ...
    assert_ne!(
        receipt.governed_scan_digest(),
        receipt.input_snapshot_digest()
    );
}

#[test]
fn verification_enforces_the_issuance_to_preparation_anchored_expiry_window() {
    let issuer = ReceiptIssuer::new(SigningKey::from_bytes(&[11; 32]));
    let receipt = receipt();
    let token = issuer
        .issue_cognition(&receipt, receipt.issued_at())
    ...
    assert!(matches!(
        verifier.verify_cognition(&token, receipt.issued_at() - TimeDelta::nanoseconds(1)),
