...
`VerifiedTypeDidContext::claims()` borrows the complete signed claim map without
allocating, so downstream boundaries can enforce count and byte budgets and
allowlist trusted names before choosing whether to construct a cloned audit
attestation. The existing `claim(name)` accessor remains the narrow path for a
single value. Both expose authenticated assertions, not automatic
authorization; application-defined claims still require the appropriate trust
source and policy validation.
...
affected IDs. An `Applied` backend outcome must carry the exact audit the vault
...
Strict cognition decoding is also a schema boundary, not an invitation to
discard data. Recursive unknown-field rejection reaches shared nested domain
types such as memory drafts, content, entities, and provenance, which may also
appear in stored or cross-service documents. Before deploying the stricter
decoder over older data, inventory and validate those documents, transform any
extra fields through an explicit versioned migration, and only then advance
readers and writers together. Future intentional fields require a schema
version and migration; they must not depend on an older reader silently
ignoring them.
