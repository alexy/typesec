...
if ! cmp -s "$cover_image" "$(cd "$(dirname "$0")/../.." && pwd)/cover/typesec-cover.png"; then
  echo "EPUB metadata check failed: packaged cover differs from cover/typesec-cover.png" >&2
  exit 1
fi
